...
A focused lead-tracking tool for sales reps with a built-in paid tier so reps can capture, score, and follow up on leads from day one.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "241082923+howardyam-cell@users.noreply.github.com"
  git config user.name "howardyam-cell"
  ```
